...
export function Pricing() {
      ...
      try {
        ...
        if (user) {
          const {
            data: subs,
            error
          } = await supabase.from('customer_subscriptions').select('*').eq('user_id', user.id).order('created_at', {
            ascending: false
          }).limit(1);
  ...
  return <section id="pricing" className="py-12 sm:py-16 lg:py-20 relative overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 rounded-2xl backdrop-blur-sm py-[48px] lg:px-[30px] my-0 bg-transparent">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <PricingCard title="Basic" price={299} interval="month" features={["Professional photography (up to 25 photos)", "Basic photo editing", "24-hour turnaround", "Digital delivery", "Limited revisions"]} description="Perfect for single-family homes and small properties" cta="Get Started" subscription={subscription} isLoading={isLoading} defaultExpanded={false} />
          <div className="relative group">
            ...
            <PricingCard title="Professional" price={499} interval="month" features={["Everything in Basic, plus:", "Up to 40 professional photos", "Drone aerial photography", "Virtual tour", "Advanced photo editing", "Social media optimized images", "Unlimited revisions"]} description="Ideal for luxury homes and medium-sized properties" cta="Go Professional" highlighted subscription={subscription} isLoading={isLoading} defaultExpanded={true} />
          </div>
          <PricingCard title="Premium" price={799} interval="month" features={["Everything in Professional, plus:", "Unlimited professional photos", "4K video tour", "3D virtual walkthrough", "Premium photo editing", "Marketing materials", "Dedicated support", "Rush delivery available"]} description="Best for luxury estates and commercial properties" cta="Go Premium" subscription={subscription} isLoading={isLoading} defaultExpanded={false} />
        </div>
      </div>
    </section>;
}
const PricingCard = ({
  ...
  return <div className={cn("relative rounded-2xl p-6 shadow-xl ring-1 ring-slate-900/10 transition-all duration-300", "bg-white hover:scale-102", "cursor-pointer overflow-hidden")} onMouseEnter={() => !isMobile && setIsExpanded(true)} onMouseLeave={() => !isMobile && setIsExpanded(false)}>
      <div className="flex items-center justify-between mb-4 relative z-10">
        <h3 className="text-lg font-semibold leading-tight text-slate-900">
          {title}
        </h3>
        {isCurrentPlan && isSubscriptionActive && <span className="px-2 py-1 text-xs font-medium text-white bg-green-500 rounded-full">
            Current Plan
          </span>}
      </div>
      ...
      <div className="flex items-center justify-center pt-4 mt-4 border-t">
        <button className="text-sm text-slate-600 hover:text-primary flex items-center gap-1 group/btn transition-all duration-300 h-11" onClick={() => setIsExpanded(!isExpanded)}>
          See Features
      ...
      <div className={cn("mt-4 overflow-hidden transition-all duration-300 relative z-10", isExpanded ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0")}>
        ...
        {isLoading ? <div className="mt-6 flex justify-center">
            <div className="animate-pulse h-11 w-full bg-gray-200 rounded-md"></div>
          </div> : isCurrentPlan && isSubscriptionActive ? <button className="mt-6 w-full py-2 px-3 bg-gray-100 text-gray-700 rounded-md text-sm font-medium" disabled>
            Current Plan
          </button> : <ShimmerButton className="mt-6 w-full h-11 sm:h-12 text-sm sm:text-base" onClick={handleSubscription} disabled={isProcessing}>

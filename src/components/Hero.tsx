...
export function Hero() {
  ...
  return <div className="w-full relative">
      ...
      <AuroraBackground className="w-full">
        <section className="flex gap-4 sm:gap-6 items-center justify-center flex-col px-4 sm:px-6 pt-0 pb-4 sm:py-8 min-h-fit sm:min-h-[60vh] relative z-10 py-[42px] my-[32px]">
          <div className="flex gap-3 sm:gap-4 flex-col max-w-5xl mx-auto w-full mt-0 sm:mt-2 my-[8px] py-0">
            <h1 className="text-[2rem] sm:text-6xl md:text-7xl tracking-tight leading-[1.1] sm:leading-[1.1] text-center sm:py-2 sm:my-2 font-bold lg:text-8xl py-0 my-0">
              ...
              <span role="text" aria-label={`Property Content that ${titles[titleNumber]}`} className="relative flex w-full justify-center h-[1.6em] sm:h-[1.8em] md:h-[1.6em] lg:h-[1.4em] overflow-hidden mt-0.5 sm:mt-2 my-0">
                {titles.map((title, index) => <motion.span key={index} className={cn("absolute font-playfair tracking-tight bg-clip-text text-transparent", "bg-gradient-to-r from-purple-700 via-blue-700 to-cyan-700", titleNumber === index && "text-[2.25rem] sm:text-6xl md:text-7xl lg:text-8xl")} initial={{
                opacity: 0,
                y: isMobile ? 15 : 40,
                scale: 0.95
          ...
          <div className="w-full sm:px-6 mt-2 sm:mt-4 mb-4 sm:mb-8 py-0 px-[27px] my-0">
            <WaitlistCTA />
          </div>
        </section>
      </AuroraBackground>

  ...
  return <div className={cn(bannerVariants({
    ...
    layout
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-1 justify-center sm:justify-start">
        ...
        <div className="flex-1 text-left sm:text-center text-xs sm:text-sm whitespace-normal px-0">
          {children}
        </div>
      </div>

      {action && <div className="flex-shrink-0 ml-2 sm:ml-4">
          {action}
        </div>}

      {isClosable && <button onClick={onClose} className="flex-shrink-0 ml-2 p-1 hover:opacity-80 transition-opacity" aria-label="Close banner">
